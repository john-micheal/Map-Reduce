...
func index(w http.ResponseWriter, req *http.Request) {
	file_bytes, err := ioutil.ReadFile("slave4.fasta")
...
func post(w http.ResponseWriter, req *http.Request) {
	...
	err = ioutil.WriteFile("slave4.fasta", b, 0644)
	if err != nil {
		log.Fatal(err)
	}
...
func handle_bases_count(w http.ResponseWriter, req *http.Request) {
	...
	f, err = os.Create("countResult4.txt")
	panicOnError(err)
	defer f.Close()
	f.WriteString(countResult)

	my_count_Result_location := "http://192.168.1.5:8094/countResult"
...
func handle_count_Result(w http.ResponseWriter, req *http.Request) {
	file_bytes, err := ioutil.ReadFile("countResult4.txt")
	fmt.Println("Handling / req")
	fmt.Fprintf(w, string(file_bytes))
	if err != nil {
		errorHandler(w, req, http.StatusInternalServerError, err)
